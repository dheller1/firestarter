...
class EntryListModel(QtGui.QStandardItemModel):
   def __init__(self, parent):
      QtGui.QStandardItemModel.__init__(self, parent)

class Entry__New(QtGui.QStandardItem):
   def __init__(self, entry=None, parent=None, iconSize=48):
...
class CategoryWidget(QtGui.QListView):
   ...
   def __init__(self, parent = None, iconSize = 48):
      ...
      # connections
      self.doubleClicked.connect(self.RunItem)
   ...
   def dropEvent(self, e):
      ...
      # since we called the base class drop event first,
      # the item has already been dropped an can be accessed
      # by its new position
      index = self.indexAt(e.pos())
      item = self.model().itemFromIndex(index)
      if item is not None:
         r = self.visualRect(index)
         nr, nc = (r.top()/self.gridSize().height(), r.left()/self.gridSize().width())
         if (nr,nc) != item.entry.position:
            item.entry.position = (nr,nc)
            if nr>self.model().rowCount(): self.model().setRowCount(nr)
            if nc>self.model().columnCount(): self.model().setColumnCount(nc)
            #self.model().takeItem(item.row(), item.column())
            #self.model().setItem(nr, nc, item)
            print item.row(), item.column()
            self.ProfileChanged.emit()
         
   def mousePressEvent(self, e):
      if self.model().itemFromIndex(self.indexAt(e.pos())) is None:
         self.clearSelection()
...
class MainWidget(QtGui.QWidget):
   ...
   def AddEntry(self, entry):
      ...
      e = Entry__New(entry, parent=self.entries)
      if entry.label == "ml2":
         self.entries.setItem(4,5,e)
      else: self.entries.appendRow(e)
         
      entry.UpdateProfile.connect(self.parent().SaveProfile)
      print self.entries.rowCount(), self.entries.columnCount()
   ...
   def InitLayout(self):
      ...
      for iconSize in (16,32,48,128,256):
         wdg = CategoryWidget(self, iconSize)
         wdg.setModel(self.entries)
   ...
   def RemoveItem(self, item):
      ...
      itm = self.entries.takeItem(item.row())
      del itm
      
      self.parent().SaveProfile()
   
   def __RemoveItem(self, entry, row):
      msg = QtGui.QMessageBox(QtGui.QMessageBox.Warning, "Warning: Deleting entry", "Do you really want to remove this entry? All"+\
                              " information and playtime will be lost and can not be restored!", QtGui.QMessageBox.Ok | QtGui.QMessageBox.Cancel, self)
      result = msg.exec_()
      if result == QtGui.QMessageBox.Cancel: return
